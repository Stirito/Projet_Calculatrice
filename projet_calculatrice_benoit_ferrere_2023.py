...
class Calculatrie(tk.Tk):
    ...
    def creer_bouton(self):
      ...
      #Boutons verts
      racine = tk.Button(self,text="√",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
      ...
      exponentielle = tk.Button(self,text="exp",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
      exponentielle.grid(**self.grid_style,column=4,row=3)

      logarithme = tk.Button(self,text="log",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
      ...
      arcsin = tk.Button(self,text="sin-1",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
      arcsin.grid(**self.grid_style,column=5,row=5)

      arccos = tk.Button(self,text="cos-1",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
      arccos.grid(**self.grid_style,column=4,row=6)

      arctan = tk.Button(self,text="tan−1",**self.button_style_vert,command=lambda: self.Appliquer(sqrt) )
    ...
    def Evaluer_Impossible(self):
       return  Pile_Vide(self.valeurNPI) or Premier(self.valeurNPI) == "**" or Premier(self.valeurNPI) in "*+-/" or Depiler(self.valeurNPI) not in "*+-/" or Depiler(self.valeurNPI) != "**"

    def EvaluerNPI(self):
      
        if self.Evaluer_Impossible:
          self.Message_Erreur("Expression Impossible à évaluer. L'écriture polonaise est requise -> Exemple : 9+9 donne 9 9 + en écriture polonaise inverse. Veuillez réessayer")
        else:
